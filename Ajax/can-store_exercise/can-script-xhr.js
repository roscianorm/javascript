...
request.onload = () => {
  if (request.status === 200) {
    ...
    console.log(
      `Network request for products.json failed with response ${request.status}: ${request.statusText}`
    );
  }
};
...
function initialize() {
  ...
  function fetchBlob(product) {
    ...
    requestImg.responseType = 'blob';

    requestImg.onload = () => {
      if (request.status === 200) {
        let img = URL.createObjectURL(requestImg.response);
        showProduct(img, product);
        console.log(img);
      } else {
        console.log(
          `Network request for ${product.name} image failed with response ${response.status}: ${response.statusText}`
        );
      }
    };
